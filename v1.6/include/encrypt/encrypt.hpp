...
class Encrypter  : public Backuper, public MD5, public AES{
 public:
    Encrypter(/* args */){};
    ~Encrypter(){};
    std::string getPSWD();                                                                           // 密码输入用星号遮掩
    bool encyptFile(std::string md5ofpwd, std::string md5md5ofpwd, std::string sourcefile, std::string targetdir);  // 对sourcefile进行aes加密，targetdir为加密文件存放目录
    bool decryptFile(std::string md5ofpwd, std::string md5md5ofpwd, std::string sourcefile, std::string targetdir); // 对sourcefile进行aes解密，targetdir为解密文件存放目录
};
