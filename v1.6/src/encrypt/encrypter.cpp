...
std::string Encrypter::getPSWD(){
    std::string password,password1;
    int flag = 1; 
    system("stty -echo");
    while (flag){
        std::cout<<"请输入密码(不显示明文)：\n";
        getline(std::cin,password);
        std::cout<<"请再次输入密码进行确认:\n";
        getline(std::cin,password1);
        if(password1==password) flag=0;
        else{
            std::cout<<"两次输入密码不符，请重新输入!\n";
        }
    }
    system("stty echo");
    return password;
}
bool Encrypter::encyptFile(std::string md5ofpwd,std::string md5ofmd5odpwd,std::string sourcefile,std::string targetdir){
...
bool Encrypter::decryptFile(std::string md5ofpwd,std::string md5md5ofpwd, std::string sourcefile,std::string targetdir){
    //验证文件后缀
    if (sourcefile.substr(sourcefile.find_last_of(".") + 1) != "en"){
        std::cout<<"文件格式不符，解密失败！\n";
        return false;
    }    
    int count,i;	
    int fin,fout;
